...
import {
  MdNotificationsNone, MdDoneAll, MdSearch, MdClose, MdInbox,
...
import {
  notificationIcon, relativeTime,
  notificationCategory, notificationCategoryKey,
  CATEGORY_THEME, CATEGORY_ORDER,
  notificationPriority, PRIORITY_THEME, PRIORITY_ORDER, isImportant,
  dateGroupKey, DATE_GROUPS,
  type NotificationCounts, type CategoryKey, type PriorityKey,
...
export default function NotificacionesPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return list.filter((it) => {
      ...
      if (categoryFilter && notificationCategoryKey(it.type, it.category) !== categoryFilter) return false;
  ...
  }, [feed, view, categoryFilter, priorityFilter, estadoFilter, dateFilter, search]);
  ...
  const categoryOptions = [
    ...
    ...CATEGORY_ORDER.map((k) => {
      ...
      return { value: k, label: n > 0 ? `${CATEGORY_THEME[k].label} (${n})` : CATEGORY_THEME[k].label };
    }),
  ];
...
function NotificationRow({
  ...
  const cat = notificationCategory(item.type, item.category);
  ...
  return (
    ...
    >
      <span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full ${cat.icon}`}>
        {createElement(notificationIcon(item.type), { size: 17 })}
      </span>

      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          {!item.read && (
            <span className="h-2 w-2 shrink-0 rounded-full" style={{ backgroundColor: cat.accent }} aria-label="No leída" />
          )}
      ...
      <span className={`hidden shrink-0 rounded-full px-2.5 py-0.5 text-[11px] font-bold sm:inline-block ${cat.pill}`}>
        {cat.label}
      </span>
